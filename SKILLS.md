...
설치 대상은 **`skills/korean-public-apis` 폴더 전체**입니다. 원본 저장소의 `scripts/`, `.github/`, 보고서, 이미지 등은 설치 대상이 아닙니다. 저장소를 포크하거나 클론하는 것만으로 스킬이 자동 설치되지는 않습니다.
...
저장소: https://github.com/contentriumkorea/public-apis-4Kr
스킬 경로: skills/korean-public-apis
...
경로는 **스킬 폴더를 넣을 부모 디렉터리**입니다. 예를 들어 Claude Code 프로젝트 경로는 `.claude/skills/korean-public-apis/SKILL.md`가 됩니다. `~`는 사용자 홈이며 Windows에서는 보통 `C:\Users\<사용자>`입니다.
...
```text
$skill-installer install the skill from https://github.com/contentriumkorea/public-apis-4Kr at skills/korean-public-apis
```

### Gemini CLI

```sh
gemini skills install https://github.com/contentriumkorea/public-apis-4Kr.git --path skills/korean-public-apis --scope user
```
...
```sh
gh skill install contentriumkorea/public-apis-4Kr skills/korean-public-apis
```
...
공식 문서의 `Customize → Rules → Add Rule → Remote Rule (Github)` 가져오기 기능 또는 위의 스킬 폴더 경로를 사용합니다. 가져온 뒤 `Customize → Skills`에서 `korean-public-apis`가 표시되는지 확인하세요.

### Claude.ai ZIP 업로드

1. [Releases](https://github.com/contentriumkorea/public-apis-4Kr/releases)에서 **`korean-public-apis.zip`로 끝나는 스킬 전용 ZIP**을 받습니다.
2. `Customize → Skills → + → Create skill → Upload a skill`에서 업로드하고 활성화합니다.
3. ZIP 안에 `korean-public-apis/SKILL.md`, `korean-public-apis/LICENSE`, `korean-public-apis/references/catalog-guide.md`가 있는지 확인합니다.

GitHub의 저장소 전체 **Download ZIP / Source code (zip)**은 스킬 전용 ZIP이 아닙니다. ZIP 이름에 날짜가 붙어 있어도 내부 스킬 폴더 이름은 `korean-public-apis`로 고정됩니다.
...
```sh
npx skills add contentriumkorea/public-apis-4Kr --skill korean-public-apis
```
...
| 도구 | 확인 방법 |
| --- | --- |
| Codex | 스킬 목록 또는 `$korean-public-apis`; 변경이 안 보이면 새 세션/재시작 |
| Claude Code | `/korean-public-apis`; 최상위 skills 디렉터리를 세션 도중 처음 만들었다면 재시작 |
| Gemini CLI | `/skills list`, 필요하면 `/skills reload` |
| Copilot CLI | `/skills reload`, `/skills info korean-public-apis` |
| VS Code / Cursor / Cline | Skills UI 또는 `/korean-public-apis`; 목록에 없으면 경로와 제품 버전 확인 |
| Windsurf Cascade | Skills UI 또는 `@korean-public-apis` |
...
- 목록의 MIT 라이선스는 개별 API의 요금, 데이터 재배포, 상업 이용을 허가하는 라이선스가 아닙니다. 원본 [LICENSE](LICENSE)와 스킬에 동봉된 [LICENSE](skills/korean-public-apis/LICENSE)를 보존하세요.
